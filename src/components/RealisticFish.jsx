...
@keyframes fishSwimRight {
  ...
  25%  { transform: translate3d(25vw, -18px, 0); }
  50%  { transform: translate3d(50vw, 12px, 0); }
  75%  { transform: translate3d(75vw, -14px, 0); }
  100% { transform: translate3d(calc(100vw + 220px), 0, 0); }
}
@keyframes fishSwimLeft {
  0%   { transform: translate3d(calc(100vw + 220px), 0, 0); }
  25%  { transform: translate3d(75vw, -18px, 0); }
  50%  { transform: translate3d(50vw, 12px, 0); }
  75%  { transform: translate3d(25vw, -12px, 0); }
...
const RealisticFish = ({ configs = defaultConfigs, className = "" }) => {
  ...
  const fish = useMemo(() => {
    if (!ready) return [];
    return isLowEnd ? configs.slice(0, 1) : configs;
  ...
  return (
    <div className={`absolute inset-0 overflow-hidden pointer-events-none ${className}`}>
      ...
      {fish.map((f, index) => (
        ...
        >
          <div style={{ willChange: "transform", animation: "fishBodySway 1.8s ease-in-out infinite", transformOrigin: "70% 50%" }}>
            <div style={{ willChange: "transform", animation: "fishWiggle 0.7s ease-in-out infinite" }}>
